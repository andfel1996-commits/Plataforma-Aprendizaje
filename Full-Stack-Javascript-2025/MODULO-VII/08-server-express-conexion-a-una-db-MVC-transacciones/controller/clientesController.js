...
// La transacción debe grantizar ATOMICIDAD
// Si ambos UPDATE  tienen exitó ---> COMMIT (Los cambios persisten juntos)
// Si cualquier UPDATE falla     ---> ROLLBACK (Ningún cambio se aplica)
export const realizarTransferencia = async (req, res , next) => {
    try {

        const emailOrigen = normalizarTexto(req.body.email_origen);
        const emailDestino = normalizarTexto(req.body.email_destino);
        const monto = validarNum(req.body.monto);

        if(!emailOrigen || !emailDestino){
            throw crearError("Los campos 'email_origen', 'email_destino' y 'monto' son obligatorios.", 400);
            
        }

        if(emailOrigen === emailDestino){
            throw crearError('El email de oriegen y destino no pueden ser iguales.', 400)
        }

        const resultado = await transferirSaldo({emailOrigen, emailDestino, monto});

        res.status(200).json({
            ok:true,
            mensaje: `Transferencia de ${monto} realizado correctamente`,
            data:resultado
        })
